...
export default function AdminPage() {
  ...
  const TIER_COLORS: Record<string, string> = { cruiser: '#aaa', builder: '#3b82f6', racer: '#a855f7', legend: '#F4A261' }
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        ...
        <button onClick={loadData} style={{ background: '#1a1a2e', border: '1px solid #2a2a3e', color: '#aaa', padding: '0.5rem 1rem', borderRadius: '0.5rem', cursor: 'pointer', fontSize: '0.875rem' }}>
          🔄 Refresh
        </button>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '1px solid #2a2a3e', paddingBottom: '0.5rem' }}>
        {TABS.map(t => (
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            {[
              ...
              { label: 'Orders', value: stats.orders, icon: '📦', color: '#F4A261' },
              { label: 'Revenue', value: `$${stats.revenue.toFixed(2)}`, icon: '💰', color: '#a855f7' },
            ].map(s => (
              <div key={s.label} style={{ background: '#1a1a2e', border: `1px solid ${s.color}25`, borderRadius: '1rem', padding: '1.25rem' }}>
      ...
      {tab === 'users' && (
        <div>
          <div style={{ background: '#1a1a2e', border: '1px solid #2a2a3e', borderRadius: '1rem', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#0D0D0D', borderBottom: '1px solid #2a2a3e' }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={u.id} style={{ borderBottom: '1px solid #1a1a2e' }}>
                    ...
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: '#F4A261' }}>⚡ {u.rev_points ?? 0}</td>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.8rem', color: '#666' }}>{new Date(u.created_at).toLocaleDateString()}</td>
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <button style={{ background: 'transparent', border: '1px solid #2a2a3e', color: '#aaa', padding: '0.25rem 0.625rem', borderRadius: '0.375rem', fontSize: '0.75rem', cursor: 'pointer' }}>View</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {(tab === 'events' || tab === 'store') && (
        <div style={{ textAlign: 'center', padding: '4rem', background: '#1a1a2e', border: '1px solid #2a2a3e', borderRadius: '1rem' }}>
