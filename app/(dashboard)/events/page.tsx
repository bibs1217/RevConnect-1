...
const TYPE_COLORS: Record<string,string> = { street_meet:'#E63946', car_show:'#FACC15', track_day:'#22c55e', cruise:'#3b82f6', drag:'#a855f7', autocross:'#ec4899', hpde:'#14b8a6' }
  ...
  return (
    <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginBottom:'1.5rem' }}>
        {TYPES.map(t => (
          <button key={t} onClick={() => setFilter(t)} style={{ padding:'0.4rem 0.875rem', borderRadius:'9999px', border:`1px solid ${filter === t ? TYPE_COLORS[t] ?? '#E63946' : '#1E3A5F'}`, background: filter === t ? `${TYPE_COLORS[t] ?? '#E63946'}15` : 'transparent', color: filter === t ? (TYPE_COLORS[t] ?? '#E63946') : '#aaa', fontSize:'0.8rem', fontWeight: filter === t ? 600 : 400, cursor:'pointer' }}>
      ...
      {/* Map placeholder */}
      <div style={{ background:'#0D1B2A', border:'1px solid #1E3A5F', borderRadius:'1rem', height:'200px', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:'1.5rem' }}>
      ...
      {showCreate && (
        <div style={{ background:'#0D1B2A', border:'1px solid #1E3A5F', borderRadius:'1rem', padding:'1.5rem', marginBottom:'1.5rem' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:'1rem' }}>
            {[['title','Event Name','DFW Summer Meet','text'],['city','City','Dallas','text'],['state','State','TX','text'],['address','Address','123 Main St','text'],['starts_at','Date & Time','','datetime-local'],['entry_fee','Entry Fee ($)','0','number']].map(([k,l,p,t]) => (
              <div key={k as string}>
                ...
                <input type={t as string} value={(newEvent as any)[k as string]} onChange={e => setNewEvent(v => ({ ...v, [k as string]: e.target.value }))} placeholder={p as string} style={{ width:'100%', background:'#0D0D0D', border:'1px solid #1E3A5F', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'white', fontSize:'0.875rem', outline:'none' }} />
              </div>
            ))}
            <div>
              <label style={{ display:'block', fontSize:'0.75rem', color:'#aaa', marginBottom:'0.375rem' }}>Event Type</label>
              <select value={newEvent.event_type} onChange={e => setNewEvent(v => ({ ...v, event_type: e.target.value }))} style={{ width:'100%', background:'#0D0D0D', border:'1px solid #1E3A5F', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'white', fontSize:'0.875rem', outline:'none' }}>
          ...
          <div style={{ display:'flex', gap:'0.75rem', marginTop:'1rem' }}>
            ...
            <button onClick={() => setShowCreate(false)} style={{ background:'transparent', color:'#aaa', border:'1px solid #1E3A5F', padding:'0.75rem 1.5rem', borderRadius:'0.75rem' }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(320px, 1fr))', gap:'1.25rem' }}>
        {filtered.map(ev => {
          ...
          return (
            <div key={ev.id} style={{ background:'#0D1B2A', border:'1px solid #1E3A5F', borderRadius:'1rem', overflow:'hidden' }}>
              ...
              <div style={{ padding:'1rem' }}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.875rem' }}>
                  ...
                  <span style={{ fontSize:'0.875rem', fontWeight:700, color: ev.entry_fee ? '#FACC15' : '#22c55e' }}>{ev.entry_fee ? `$${ev.entry_fee}` : 'Free'}</span>
                </div>
                <div style={{ display:'flex', gap:'0.5rem' }}>
                  ...
                  <button style={{ flex:1, background:'transparent', border:'1px solid #1E3A5F', color:'#aaa', padding:'0.5rem', borderRadius:'0.5rem', fontSize:'0.8rem', cursor:'pointer' }}>Details</button>
                </div>
              </div>
            </div>
          )
