  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        ...
        <button onClick={loadData} style={{ background: '#152234', border: '1px solid #1E3A6E', color: '#aaa', padding: '0.5rem 1rem', borderRadius: '0.5rem', cursor: 'pointer', fontSize: '0.875rem' }}>
          🔄 Refresh
        </button>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '1px solid #1E3A6E', paddingBottom: '0.5rem' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem 0.5rem 0 0', border: 'none', background: tab === t.id ? '#CC0000' : 'transparent', color: tab === t.id ? 'white' : '#aaa', fontWeight: tab === t.id ? 700 : 400, fontSize: '0.875rem', cursor: 'pointer' }}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            {[
              ...
              { label: 'Vehicles', value: stats.vehicles, icon: '🚗', color: '#CC0000' },
              ...
              <div key={s.label} style={{ background: '#152234', border: `1px solid ${s.color}25`, borderRadius: '1rem', padding: '1.25rem' }}>
      ...
      {tab === 'users' && (
        <div>
          <div style={{ background: '#152234', border: '1px solid #1E3A6E', borderRadius: '1rem', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#0D0D0D', borderBottom: '1px solid #1E3A6E' }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={u.id} style={{ borderBottom: '1px solid #152234' }}>
                    ...
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <button style={{ background: 'transparent', border: '1px solid #1E3A6E', color: '#aaa', padding: '0.25rem 0.625rem', borderRadius: '0.375rem', fontSize: '0.75rem', cursor: 'pointer' }}>View</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {(tab === 'events' || tab === 'store') && (
        <div style={{ textAlign: 'center', padding: '4rem', background: '#152234', border: '1px solid #1E3A6E', borderRadius: '1rem' }}>
