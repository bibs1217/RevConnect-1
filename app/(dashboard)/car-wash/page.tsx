...
const PRICE_COLORS: Record<string,string> = { '$':'#22c55e', '$$':'#F4A261', '$$$':'#E63946', '$$$$':'#a855f7' }
  ...
  return (
    <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
      ...
      <div style={{ background:'rgba(244,162,97,0.08)', border:'1px solid rgba(244,162,97,0.2)', borderRadius:'0.75rem', padding:'0.875rem 1rem', marginBottom:'1rem', display:'flex', gap:'0.75rem' }}>
        <span style={{ color:'#F4A261', fontSize:'1.1rem' }}>⚠️</span>
        <div>
          <p style={{ fontSize:'0.875rem', fontWeight:600, color:'#F4A261' }}>Coating protection active</p>
      ...
      <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginBottom:'1rem', alignItems:'center' }}>
        {['All','tunnel_touchless','tunnel_soft','hand_wash','mobile_detailer','full_detail'].map(t => (
          <button key={t} onClick={() => setFilterType(t)} style={{ padding:'0.375rem 0.75rem', borderRadius:'9999px', border:`1px solid ${filterType===t ? '#E63946' : '#2a2a3e'}`, background: filterType===t ? 'rgba(230,57,70,0.1)' : 'transparent', color: filterType===t ? '#E63946' : '#aaa', fontSize:'0.8rem', cursor:'pointer' }}>
      ...
      {/* Map */}
      <div style={{ background:'#1a1a2e', border:'1px solid #2a2a3e', borderRadius:'1rem', height:'180px', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:'1.5rem' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns: selected ? '1fr 1fr' : 'repeat(auto-fill, minmax(320px, 1fr))', gap:'1.25rem' }}>
        <div style={{ display:'grid', gridTemplateColumns: selected ? '1fr' : 'repeat(auto-fill, minmax(300px, 1fr))', gap:'1rem' }}>
          {filtered.map(w => (
            <div key={w.id} onClick={() => setSelected(w)} style={{ background:'#1a1a2e', border:`1px solid ${selected?.id === w.id ? '#E63946' : '#2a2a3e'}`, borderRadius:'1rem', padding:'1rem', cursor:'pointer' }}>
              ...
              <p style={{ fontSize:'0.8rem', color:'#F4A261', marginBottom:'0.625rem' }}>⭐ {w.rating} ({w.reviews} reviews) · {w.hours}</p>
              <div style={{ display:'flex', gap:'0.375rem', flexWrap:'wrap' }}>
                ...
                {w.membership && <span style={{ background:'rgba(244,162,97,0.1)', color:'#F4A261', padding:'0.15rem 0.5rem', borderRadius:'9999px', fontSize:'0.7rem', border:'1px solid rgba(244,162,97,0.2)' }}>Membership</span>}
              </div>
            </div>
          ))}
        </div>
        ...
        {selected && (
          <div style={{ background:'#1a1a2e', border:'1px solid #E63946', borderRadius:'1rem', padding:'1.5rem', height:'fit-content' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'0.625rem' }}>
              ...
              <button style={{ background:'transparent', border:'1px solid #2a2a3e', color:'#aaa', padding:'0.625rem', borderRadius:'0.75rem', cursor:'pointer' }}>Report Damage Issue</button>
            </div>
          </div>
        )}
      </div>
