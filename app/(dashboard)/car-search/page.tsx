...
{
name: 'Porsche CPO', emoji: '磊', color: '#C9A84C',
url: ({ zip }) => {
const q = qp([['zip', zip]])
...
{
name: 'CarBrain', emoji: '易', color: '#FF6600',
url: ({ zip }) => {
const q = qp([['zip', zip]])
...
const [aiListings, setAiListings] = useState<AiCar[]>([])
...
async function fetchAIListings(f: Filters) {
setAiLoading(true)
setAiError('')
...
if (f.zip) p.set('zip', f.zip.replace(/\D/g, ''))
try {
const res = await fetch(`/api/ai-car-search?${p}`)
...
Live listings from eBay Motors · AI-powered results · Direct links to 50+ car sites
</div>
</div>
...
style={{ background: (loading || aiLoading) ? BORDER : RED, color: '#fff', border: 'none', borderRadius: 8, padding: '10px 36px', fontSize: 15, fontWeight: 700, cursor: (loading || aiLoading) ? 'not-allowed' : 'pointer' }}>
{(loading || aiLoading) ? 'Searching…' : ' Search Cars'}
</button>
</div>
</div>
</form>
...
{/* ── AI-Powered Listings ── */}
{(aiLoading || (searched && aiListings.length > 0) || aiError) && (
<div style={{ marginBottom: 28 }}>
<div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
<span style={{ fontSize: 18 }}>烙</span>
<span style={{ fontSize: 17, fontWeight: 700 }}>AI-Powered Listings</span>
<span style={{ background: 'rgba(255,215,0,0.12)', border: `1px solid ${GOLD}44`, color: GOLD, fontSize: 11, padding: '2px 10px', borderRadius: 20, fontWeight: 700 }}>
AI AGENT
</span>
{aiLoading && <span style={{ color: MUTED, fontSize: 13 }}>Generating personalized results…</span>}
</div>

{aiLoading && (
<div style={{ background: CARD, borderRadius: 10, padding: '32px 0', textAlign: 'center', border: `1px solid ${BORDER}` }}>
<div style={{ fontSize: 28, marginBottom: 8 }}>⚙️</div>
<div style={{ color: MUTED, fontSize: 14 }}>AI agent is searching for matching vehicles…</div>
</div>
)}

{aiError && !aiLoading && (
<div style={{ background: '#3A1010', border: `1px solid ${RED}`, borderRadius: 8, padding: '10px 14px', color: '#FF8080', fontSize: 13 }}>
{aiError}
</div>
)}

{!aiLoading && aiListings.length > 0 && (
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(270px,1fr))', gap: 14 }}>
{aiListings.map(car => (
<div key={car.id}
style={{ background: CARD, borderRadius: 10, overflow: 'hidden', border: `1px solid #FFD70033`, display: 'flex', flexDirection: 'column' }}>
{/* AI Badge header */}
<div style={{ background: 'rgba(255,215,0,0.08)', borderBottom: `1px solid #FFD70022`, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 6 }}>
<span style={{ fontSize: 11, fontWeight: 700, color: GOLD }}>烙 AI MATCHED</span>
<span style={{ fontSize: 11, color: MUTED, marginLeft: 'auto' }}>{car.condition}</span>
</div>
{/* Placeholder photo area */}
<div style={{ height: 140, background: `linear-gradient(135deg, #0E1825 0%, #1B2A3E 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
<span style={{ fontSize: 48 }}></span>
<span style={{ position: 'absolute', bottom: 8, right: 10, fontSize: 11, color: MUTED }}>
{car.year} · {car.mileage ? car.mileage.toLocaleString() + ' mi' : 'Miles N/A'}
</span>
</div>
{/* Info */}
<div style={{ padding: '10px 13px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
<div style={{ fontWeight: 700, fontSize: 14, color: TEXT, lineHeight: 1.3 }}>{car.title}</div>
<div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
<span style={{ fontSize: 22, fontWeight: 800, color: GOLD }}>
{car.price > 0 ? `$${car.price.toLocaleString()}` : 'Call for price'}
</span>
{car.price > 0 && <span style={{ color: MUTED, fontSize: 12 }}>{monthly(car.price)}</span>}
</div>
<div style={{ fontSize: 12, color: TEXT, opacity: 0.7, lineHeight: 1.4 }}>{car.description}</div>
<div style={{ display: 'flex', gap: 12, fontSize: 12, color: MUTED, flexWrap: 'wrap' }}>
{car.location && <span> {car.location}</span>}
{car.seller && <span> {car.seller}</span>}
</div>
<div style={{ marginTop: 'auto', paddingTop: 8 }}>
<a href={car.url} target="_blank" rel="noopener noreferrer"
style={{ display: 'block', background: `linear-gradient(135deg, ${GOLD}, #E6B800)`, color: '#000', padding: '8px 0', borderRadius: 6, textAlign: 'center', textDecoration: 'none', fontSize: 13, fontWeight: 700 }}>
View Listing ↗
</a>
</div>
</div>
...
<div style={{ fontSize: 18, color: TEXT, marginBottom: 6 }}>Enter make and model to search</div>
<div style={{ fontSize: 13 }}>Get AI-powered + eBay Motors listings, or click any site below to browse directly.</div>
</div>
)}
