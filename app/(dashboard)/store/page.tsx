...
export default function StorePage() {
  ...
  return (
    <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
      {/* Header */}
      <div style={{ background:'linear-gradient(135deg, rgba(204,0,0,0.12), rgba(244,162,97,0.06))', border:'1px solid rgba(204,0,0,0.2)', borderRadius:'1rem', padding:'1.5rem', marginBottom:'1.5rem', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem' }}>
        <div>
        ...
        <div style={{ display:'flex', gap:'1rem', alignItems:'center' }}>
          ...
          {cartCount > 0 && (
            <div style={{ background:'#CC0000', color:'white', padding:'0.5rem 1rem', borderRadius:'0.75rem', fontWeight:700, cursor:'pointer', display:'flex', gap:'0.5rem', alignItems:'center' }}>
      ...
      <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginBottom:'1.5rem' }}>
        {CATS.map(c => (
          <button key={c} onClick={() => setCat(c)} style={{ padding:'0.375rem 0.875rem', borderRadius:'9999px', border:`1px solid ${cat===c ? '#CC0000' : '#1E3A6E'}`, background: cat===c ? 'rgba(204,0,0,0.1)' : 'transparent', color: cat===c ? '#CC0000' : '#aaa', fontSize:'0.8rem', cursor:'pointer' }}>{c}</button>
        ))}
      </div>
      ...
      {selected && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem' }} onClick={() => setSelected(null)}>
          <div style={{ background:'#152234', border:'1px solid #1E3A6E', borderRadius:'1.25rem', padding:'2rem', maxWidth:'600px', width:'100%' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ height:'200px', background:'linear-gradient(135deg, rgba(204,0,0,0.08), transparent)', borderRadius:'0.75rem', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'5rem', marginBottom:'1.25rem' }}>{selected.img}</div>
              ...
              <div style={{ marginBottom:'1rem' }}>
                ...
                <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap' }}>
                  {selected.sizes.map(s => (
                    <button key={s} onClick={() => setSelectedSize(s)} style={{ padding:'0.375rem 0.875rem', borderRadius:'0.5rem', border:`1px solid ${selectedSize===s ? '#CC0000' : '#1E3A6E'}`, background: selectedSize===s ? 'rgba(204,0,0,0.1)' : 'transparent', color: selectedSize===s ? '#CC0000' : '#aaa', fontSize:'0.8rem', cursor:'pointer' }}>{s}</button>
                  ))}
                </div>
              </div>
            )}
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem' }}>
              <div>
                <p style={{ fontSize:'1.75rem', fontWeight:900, color:'#CC0000' }}>${selected.price}</p>
                <p style={{ fontSize:'0.75rem', color:tierData.color }}>+{Math.round(selected.revPoints * tierData.mult)} Rev Points</p>
              </div>
              {selected.isLimited && <span style={{ background:'rgba(204,0,0,0.1)', border:'1px solid rgba(204,0,0,0.2)', color:'#CC0000', padding:'0.25rem 0.75rem', borderRadius:'9999px', fontSize:'0.8rem', fontWeight:600 }}>Limited Drop</span>}
            </div>
            <button onClick={() => { addToCart(selected.id); setSelected(null) }} style={{ width:'100%', background:'#CC0000', color:'white', border:'none', padding:'0.875rem', borderRadius:'0.875rem', fontWeight:700, fontSize:'1rem', cursor:'pointer' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(260px, 1fr))', gap:'1.25rem' }}>
        {filtered.map(p => {
          ...
          return (
            <div key={p.id} style={{ background:'#152234', border:`1px solid ${p.isLimited ? 'rgba(204,0,0,0.3)' : '#1E3A6E'}`, borderRadius:'1rem', overflow:'hidden', cursor:'pointer' }} onClick={() => { setSelected(p); setSelectedSize('') }}>
              <div style={{ height:'180px', background:'linear-gradient(135deg, rgba(204,0,0,0.06), transparent)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'5rem', position:'relative' }}>
                {p.img}
                <div style={{ position:'absolute', top:'0.625rem', left:'0.625rem', display:'flex', gap:'0.375rem', flexWrap:'wrap' }}>
                  ...
                  {p.isLimited && <span style={{ background:'#CC0000', color:'white', padding:'0.15rem 0.5rem', borderRadius:'9999px', fontSize:'0.65rem', fontWeight:700 }}>LIMITED</span>}
                </div>
              </div>
              <div style={{ padding:'1rem' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <div>
                    <span style={{ fontWeight:800, color:'#CC0000', fontSize:'1.1rem' }}>${p.price}</span>
                  ...
                  <button onClick={e => { e.stopPropagation(); addToCart(p.id) }} style={{ background: isAdded ? 'rgba(34,197,94,0.15)' : 'rgba(204,0,0,0.1)', border:`1px solid ${isAdded ? 'rgba(34,197,94,0.3)' : 'rgba(204,0,0,0.2)'}`, color: isAdded ? '#22c55e' : '#CC0000', padding:'0.4rem 0.625rem', borderRadius:'0.5rem', fontSize:'0.8rem', fontWeight:600, cursor:'pointer', transition:'all 0.15s' }}>
                    {isAdded ? '✓ Added' : '+ Cart'}
                  </button>
                </div>
              </div>
