...
const SOURCE_COLORS: Record<string,string> = { 'Bring a Trailer':'#E63946','Cars & Bids':'#F4A261','Copart':'#3b82f6','eBay Motors':'#a855f7','Mecum':'#22c55e' }
    ...
    <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginBottom:'1.5rem' }}>
        {['All','collector','public','online','dealer'].map(t => (
          <button key={t} onClick={() => setFilter(t)} style={{ padding:'0.375rem 0.875rem', borderRadius:'9999px', border:`1px solid ${filter===t ? '#E63946' : '#2a2a3e'}`, background: filter===t ? 'rgba(230,57,70,0.1)' : 'transparent', color: filter===t ? '#E63946' : '#aaa', fontSize:'0.8rem', cursor:'pointer', textTransform:'capitalize' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns: selected ? '1fr 380px' : 'repeat(auto-fill, minmax(340px, 1fr))', gap:'1.25rem' }}>
        <div style={{ display:'grid', gridTemplateColumns: selected ? '1fr' : 'repeat(auto-fill, minmax(320px, 1fr))', gap:'1rem' }}>
          {filtered.map(a => {
            ...
            return (
              <div key={a.id} onClick={() => setSelected(a)} style={{ background:'#1a1a2e', border:`1px solid ${selected?.id===a.id ? '#E63946' : '#2a2a3e'}`, borderRadius:'1rem', overflow:'hidden', cursor:'pointer' }}>
                <div style={{ height:'140px', background:'linear-gradient(135deg, rgba(230,57,70,0.08), transparent)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'4rem', position:'relative' }}>
                  ...
                  <span style={{ position:'absolute', top:'0.5rem', right:'0.5rem', background:'rgba(0,0,0,0.6)', padding:'0.2rem 0.5rem', borderRadius:'9999px', fontSize:'0.7rem', color: timeLeft(a.ends_at).includes('h') || timeLeft(a.ends_at).includes('m') ? '#E63946' : '#F4A261' }}>{timeLeft(a.ends_at)}</span>
                </div>
                <div style={{ padding:'1rem' }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end' }}>
                    <div>
                      ...
                      <p style={{ fontSize:'0.75rem', color:'#F4A261' }}>All-in: ~${Math.round(allIn).toLocaleString()}</p>
                    </div>
                    <div style={{ textAlign:'right' }}>
                      ...
                      {a.reserve_met === null && <span style={{ background:'rgba(244,162,97,0.1)', color:'#F4A261', padding:'0.2rem 0.5rem', borderRadius:'9999px', fontSize:'0.7rem', border:'1px solid rgba(244,162,97,0.2)' }}>Reserve?</span>}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {selected && (
          <div style={{ background:'#1a1a2e', border:'1px solid #E63946', borderRadius:'1rem', padding:'1.5rem', height:'fit-content', position:'sticky', top:'5rem' }}>
            ...
            <div style={{ background:'#0D0D0D', borderRadius:'0.75rem', padding:'1rem', marginBottom:'1rem' }}>
              ...
              <div style={{ height:'1px', background:'#2a2a3e', margin:'0.5rem 0' }} />
            ...
            <div style={{ marginBottom:'1rem' }}>
              ...
              <input value={bidAmount} onChange={e => setBidAmount(e.target.value)} placeholder="Enter your max bid" type="number" style={{ width:'100%', background:'#0D0D0D', border:'1px solid #2a2a3e', borderRadius:'0.625rem', padding:'0.625rem 0.875rem', color:'white', fontSize:'0.9rem', outline:'none' }} />
              {bidAmount && (
                <p style={{ fontSize:'0.8rem', color:'#F4A261', marginTop:'0.375rem' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'0.625rem' }}>
              ...
              <button style={{ background:'transparent', border:'1px solid #2a2a3e', color:'#aaa', padding:'0.625rem', borderRadius:'0.75rem', cursor:'pointer', fontSize:'0.8rem' }}>♡ Watch This Lot</button>
            </div>
          </div>
        )}
      </div>
